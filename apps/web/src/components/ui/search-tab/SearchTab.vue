...
import { CaseSensitive, ChevronDown, ChevronRight, ChevronUp, Regex, Replace, ReplaceAll, TextSelect, WholeWord, X } from '@lucide/vue'
...
/** A whole-word hit has no word characters directly adjacent to the match. */
...
function handleReplace() {
  ...
  findAllMatches()
  toast.success(t(`search.replacedOne`, { remaining: matchPositions.value.length }))
}
...
function handleReplaceAll() {
  ...
  const replacedCount = matchPositions.value.length
      ...
      <div class="grid min-w-0 flex-1 grid-cols-[1fr_auto] items-center gap-0.5">
        <div class="relative min-w-0">
          <Input
            ...
            class="h-7 w-full min-w-0 pr-20 text-sm"
