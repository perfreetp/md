...
export function postSignature(post: Post): string {
  return `${post.id}:${post.title}:${post.content.length}:${post.updateDatetime.getTime()}:${post.parentId ?? ``}:${post.history?.length ?? 0}:${post.history?.[0]?.datetime ?? ``}:${post.collapsed ? 1 : 0}`
}
